...
## Tabla Resumen
IMPORTANTE: Esta tabla SIEMPRE debe incluirse completa en la respuesta final al agente principal.
Es la referencia que el usuario usa para identificar hallazgos por ID. Nunca omitirla ni resumirla.

| ID | Severidad | Hallazgo | Req. ASVS | Archivo:Línea | Aplica a | Estado |
|----|-----------|----------|-----------|---------------|----------|--------|
| C1 | 🔴 CRÍTICO | ... | V2.1.1 | archivo.java:42 | Producción + Local | Abierto |

La tabla DEBE incluir TODAS las columnas: ID, Severidad, Hallazgo, Req. ASVS, Archivo:Línea, Aplica a, Estado.
Nunca omitir las columnas "Archivo:Línea" ni "Aplica a" — son esenciales para que el usuario localice y priorice los hallazgos.
